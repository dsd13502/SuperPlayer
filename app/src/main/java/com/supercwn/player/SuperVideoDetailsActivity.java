 ...
 * @time 2016-9-19
 */
public class SuperVideoDetailsActivity extends AppCompatActivity implements View.OnClickListener, SuperPlayer.OnNetChangeListener, ITryAndSeeEndListener {
    ...
    private void initPlayer() {
        ...
        player.setOnTryAndSeeEndListener(this);
        if (isLive) {
        ...
        player.showCenterControl(true);//显示控制中心
...
//                player.play(url);
    ...
    @Override
    public void OnTryAndSeeEndController() {
        if (player.isPlaying()){
            player.pause();
            Toast.makeText(this,"视频时间到",Toast.LENGTH_SHORT).show();
        }
    }
}
